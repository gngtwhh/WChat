...
func (s *GroupService) CreateGroup(
    ...
    // groupUUID := strings.ReplaceAll(uuid.New().String(), "-", "")[:20]
    ...
    for _, m := range initialMembers {
        ...
        // 必须是好友
        isFriend, err := s.contactRepo.CheckContactExists(ctx, ownerID, m)
        if err != nil {
            return "", err
        }
        if !isFriend {
    ...
    if err := s.groupRepo.CreateGroupTx(ctx, group, finalMembers); err != nil {
        return "", err
    }
...
func (s *GroupService) UpdateGroupInfo(ctx context.Context, operatorID, groupUUID string, dto GroupUpdateDTO) error {
    ...
    return s.groupRepo.UpdateGroup(ctx, groupUUID, updateData)
}
...
func (s *GroupService) DismissGroup(ctx context.Context, operatorID, groupUUID string) error {
    ...
    return s.groupRepo.DismissGroupTx(ctx, groupUUID)
}
...
func (s *GroupService) InviteToGroup(ctx context.Context, inviterID, groupUUID string, userIDs []string) error {
    ...
    for _, uid := range userIDs {
        ...
        // 必须是好友
        isFriend, err := s.contactRepo.CheckContactExists(ctx, inviterID, uid)
        if err != nil {
            return err
        }
        if !isFriend {
    ...
    return s.groupRepo.AddMembersTx(ctx, groupUUID, userIDs)
}
...
func (s *GroupService) LeaveGroup(ctx context.Context, userID, groupUUID string) error {
    ...
    return s.groupRepo.RemoveMemberTx(ctx, groupUUID, userID, 6)
}
...
func (s *GroupService) KickMember(ctx context.Context, operatorID, groupUUID, targetUserID string) error {
    ...
    return s.groupRepo.RemoveMemberTx(ctx, groupUUID, targetUserID, 7)
}
