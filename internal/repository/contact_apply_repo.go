...
// FindReceivedByUser 获取该用户收到的所有申请
// 包含了别人加自己的(ContactType=0)，以及别人加自己群的(ContactType=1)
func (r *ContactApplyRepo) FindReceivedByUser(ctx context.Context, userID string) ([]model.ContactApply, error) {
...
// FindByUUID 查询单条申请
func (r *ContactApplyRepo) FindByUUID(ctx context.Context, uuid string) (*model.ContactApply, error) {
...
// Save 保存或更新申请记录 (如果之前有拒绝的记录，重新发起时要更新时间和状态)
func (r *ContactApplyRepo) Save(ctx context.Context, apply *model.ContactApply) error {
	return r.db.WithContext(ctx).Save(apply).Error
}

// FindByUserAndContact 查找双方是否已经有过申请记录
func (r *ContactApplyRepo) FindByUserAndContact(
...
// HandleUserApply 处理好友申请 (更新申请状态 + 建立双向好友关系)
func (r *ContactApplyRepo) HandleUserApply(
...
func (r *ContactApplyRepo) DeleteByUserUUID(ctx context.Context, uuid string) error {
	return r.db.WithContext(ctx).
		Where("user_id = ? OR (contact_type = 0 AND contact_id = ?)", uuid, uuid).
		Delete(&model.ContactApply{}).Error
}
