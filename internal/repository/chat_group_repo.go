...
func (r *GroupRepo) CreateWithMembers(ctx context.Context, group *model.Group, userIDs []string) error {
...
func (r *GroupRepo) FindActiveByUUID(ctx context.Context, uuid string) (*model.Group, error) {
...
func (r *GroupRepo) UpdateInfoByUUID(ctx context.Context, groupUUID string, data map[string]any) error {
	return r.db.WithContext(ctx).Model(&model.Group{}).
		Where("uuid = ?", groupUUID).Updates(data).Error
}

func (r *GroupRepo) DismissByUUID(ctx context.Context, groupUUID string) error {
	return r.db.WithContext(ctx).Transaction(
		func(tx *gorm.DB) error {
			// 修改群状态为 2 (解散)
...
func (r *GroupRepo) AddMembers(ctx context.Context, groupUUID string, newMemberIDs []string) error {
	return r.db.WithContext(ctx).Transaction(
		func(tx *gorm.DB) error {
...
func (r *GroupRepo) RemoveMember(ctx context.Context, groupUUID, memberUUID string, contactStatus int8) error {
	return r.db.WithContext(ctx).Transaction(
		func(tx *gorm.DB) error {
			var group model.Group
			if err := tx.Clauses(clause.Locking{Strength: clause.LockingStrengthUpdate}).Where(
...
func (r *GroupRepo) DismissOwnedGroups(ctx context.Context, ownerUUID string) error {
	var groupUUIDs []string
	if err := r.db.WithContext(ctx).
		Model(&model.Group{}).
		Where("owner_id = ? AND status <> 2", ownerUUID).
		Pluck("uuid", &groupUUIDs).Error; err != nil {
		return err
	}
	if len(groupUUIDs) == 0 {
		return nil
	...
	return r.db.WithContext(ctx).
		Model(&model.Contact{}).
		Where("contact_type = 1 AND contact_id IN ?", groupUUIDs).
		Update("status", 6).Error
}

func (r *GroupRepo) RemoveUserFromJoinedGroups(ctx context.Context, userUUID string) error {
	groups, err := r.FindJoinedGroups(ctx, userUUID)
	if err != nil {
		return err
	}

	for i := range groups {
		if groups[i].OwnerId == userUUID {
			continue
		}
		var members []string
		if err := json.Unmarshal(groups[i].Members, &members); err != nil {
			return err
		}
		...
		for _, memberUUID := range members {
			...
			nextMembers = append(nextMembers, memberUUID)
		}
		if !removed {
			continue
		}
		...
		if err := r.db.WithContext(ctx).
			Model(&model.Group{}).
			Where("uuid = ?", groups[i].Uuid).
			Updates(
				map[string]any{
					"members":    nextMembersJSON,
					"member_cnt": len(nextMembers),
				},
			).Error; err != nil {
			return err
		}
		if err := r.db.WithContext(ctx).
			Model(&model.Contact{}).
			Where("user_id = ? AND contact_id = ? AND contact_type = 1", userUUID, groups[i].Uuid).
			Update("status", 6).Error; err != nil {
			return err
		}
	}
