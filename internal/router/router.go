...
// LoadRouters 加载所有路由
func LoadRouters(app *handler.App, authSvc *service.AuthService) *gin.Engine {
	...
	protected.Use(middleware.Auth(authSvc))
	{
		...
		{
			auth.POST("/logout", app.Auth.Logout)
		}
		...
		{
			me.GET("", app.User.GetMyProfile)                            // 获取当前用户资料
			me.PUT("", app.User.UpdateMyProfile)                         // 修改当前用户资料
			me.POST("/phone/change", app.User.ChangeTelephone)           // 更换手机号
			me.POST("/delete-request", app.User.RequestAccountDeletion)  // 发起注销申请
			me.DELETE("/delete-request", app.User.CancelAccountDeletion) // 取消注销申请
		}
